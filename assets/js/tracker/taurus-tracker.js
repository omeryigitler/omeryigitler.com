...
(function () {
    ...
    // --- 6. VOICE COMMAND CONTROL (THE MISSING LINK) ---
    function initVoiceControl() {
        ...
        recognition.lang = 'tr-TR'; // Turkish Default

        recognition.onstart = () => console.log("🎤 Taurus Voice Listner Active");
        recognition.onerror = (event) => console.log("🎤 Voice Error:", event.error);

        recognition.onend = () => {
            // Auto-restart listener (Always On)
        ...
        recognition.onresult = (event) => {
            ...
            // LOGIC: "Durdur" / "Freeze"
            if (command.includes("durdur") || command.includes("stop") || command.includes("don")) {
                console.log("🎤 EXECUTING: FREEZE");
                handleCommand({ action: 'freeze', message: null }); // Local Instant Trigger

                // Optional: Sync to DB so other devices see it? 
                // For now, client-side is fast and user requested "Why can't I write"
            }

            // LOGIC: "Yaz" / "Write"
            // Pattern: "Ekrana [Mesaj] yaz" or "Durdur ve [Mesaj] yaz"
            if (command.includes("yaz")) {
                // Extract message: Everything before "yaz"
                // Example: "Ekrana merhaba dünya yaz" -> "merhaba dünya"
                // Heuristic: remove "ekrana", remove "yaz", remove "durdur ve"
                let msg = command.replace("ekrana", "")
                    .replace("yaz", "")
                    .replace("durdur ve", "")
                    .replace("lütfen", "")
                    .trim();
                ...
                if (msg.length > 0) {
                    console.log("🎤 EXECUTING: TYPE ->", msg);
                    handleCommand({ action: 'freeze', message: msg });
                }
            }

            // LOGIC: "Devam" / "Clear"
            if (command.includes("devam") || command.includes("iptal") || command.includes("aç")) {
                handleCommand('clear');
            }
        };

        // Start listening after user interaction (browser policy)
        // We'll try auto-start, but fallback to click
        try {
            recognition.start();
        } catch (e) {
