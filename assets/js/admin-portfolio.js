(() => {
  ...
  const state = { mounted: false, editingId: null, projects: [], capturing: new Set() };
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .pf-list{display:grid;gap:10px;margin-top:22px}.pf-row{display:grid;grid-template-columns:74px minmax(0,1fr) auto;align-items:center;gap:12px;padding:10px;border:1px solid rgba(255,255,255,.08);border-radius:14px;background:rgba(255,255,255,.025)}.pf-thumb{width:74px;height:48px;border:1px solid rgba(255,255,255,.1);border-radius:8px;background:#0b0b0b;object-fit:cover;object-position:top}.pf-title{color:#fff;font:700 12px Manrope,sans-serif}.pf-meta{margin-top:4px;color:#777;font:700 9px "JetBrains Mono",monospace;letter-spacing:.08em;text-transform:uppercase;line-height:1.5}.pf-status{color:#22c55e}.pf-status.hidden{color:#888}.pf-row-actions{display:flex;gap:7px}.pf-icon{width:34px;height:34px;border:1px solid rgba(255,255,255,.1);border-radius:9px;background:rgba(255,255,255,.04);color:#ccc;cursor:pointer}.pf-icon:hover{border-color:rgba(255,215,0,.4);color:#FFD700}.pf-icon.capture{color:#86efac}.pf-icon.delete:hover{border-color:rgba(248,113,113,.4);color:#f87171}.pf-icon:disabled{opacity:.4;cursor:wait}.pf-empty{padding:22px;border:1px dashed rgba(255,255,255,.12);border-radius:14px;color:#777;text-align:center;font:700 10px "JetBrains Mono",monospace;letter-spacing:.1em;text-transform:uppercase}
      @media(max-width:720px){.pf-grid{grid-template-columns:1fr}.pf-field.wide{grid-column:auto}.pf-head{flex-direction:column}.pf-row{grid-template-columns:58px minmax(0,1fr)}.pf-thumb{width:58px}.pf-row-actions{grid-column:1/-1;justify-content:flex-end}}
    `;
  ...
  function render(projects) {
    ...
    list.innerHTML = projects.map((project) => {
      ...
      return `<article class="pf-row"><img class="pf-thumb" src="${esc(project.desktopImage || "assets/preview.png")}" alt=""><div><div class="pf-title">${esc(project.title || "Untitled")}</div><div class="pf-meta"><span class="pf-status${project.published === false ? " hidden" : ""}">${project.published === false ? "Hidden" : "Published"}</span> · ${esc(captureLabel)} · order ${esc(project.sortOrder ?? 999)}</div></div><div class="pf-row-actions"><button class="pf-icon capture" type="button" data-capture="${esc(project.id)}" title="Capture first desktop and mobile viewport" ${busy ? "disabled" : ""}>↻</button><button class="pf-icon" type="button" data-edit="${esc(project.id)}" title="Edit">✎</button><button class="pf-icon delete" type="button" data-delete="${esc(project.id)}" title="Delete">×</button></div></article>`;
    }).join("");
  ...
  async function seed(showNotice = true) {
    try { await request({ op: "seed", projects: defaults }); await load(); if (showNotice) await alertUser("PORTFOLIO RESTORED", "The current four case studies are available. Use Refresh all previews to capture their current first screens.", "database"); }
