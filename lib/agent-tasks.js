...
function stripKeywords(text, extraPatterns = []) {
  ...
  const patterns = [
    ...
    /saat\s+\d{1,2}\b/gi,
    /\d{1,2}[./]\d{1,2}(?:[./]\d{2,4})?/g,
    // polite/filler tails: "... diye not ekler misin", "lütfen", "rica etsem"
    /\b(diye|deiye|şeklinde|seklinde)\b/gi,
    /\b(m[ıiuü]s[ıiuü]n(ız|iz|uz|üz)?|l[üu]tfen|rica\s+\w*)\b/gi,
    /\bnot(u|unu)?\b/gi,
    ...extraPatterns
  ];
...
// Static map of the admin panel: every section and the controls inside it,
// extracted from admin.html so chat answers can reference the real UI.
const PANEL_GUIDE = [
  "DASHBOARD: istatistik kartları (toplam görüntülenme, toplam mesaj, proje sayısı, sistem durumu), Traffic grafiği, Recent Leads listesi.",
  "INBOX (Communications Hub): müşteri mesajları tablosu. Filtreler: All Messages / Unread. Kısımlar: toplu seçim + Delete Selected, satırda durum rozeti (NEW/ARCHIVED) ve QUOTED/PROJECT etiketleri, gönderen, mesaj, tarih, işlemler (okundu işaretle, sil). Mesaj gelince müşteri kaydı otomatik oluşturulur/bağlanır.",
  "PRICING: gömülü fiyatlandırma aracı (pricing-tool). Ülke/site tipi/tasarım/sayfa/özellik/ek hizmet/teslim/bakım seçimiyle fiyat hesaplar; 'Save Quote' panele taslak teklif olarak düşer. Yeni sekmede de açılabilir.",
  "PROPOSALS: teklif listesi. Filtreler: All / Draft / Sent / Accepted. Kısımlar: toplu silme, mesajdan teklif oluşturma, PDF üretme (sunucuda /api/pdf), gönderme. Akış: Draft → Sent → Accepted. Numaralar Q-YYYY-NNN; agent onayıyla oluşanlar QA-YYYY-XXXX.",
  "PROJECTS: proje kartları (başlık, durum rozeti, müşteri, aşama + ilerleme çubuğu, bütçe, teslim tarihi). Filtreler: Active / Archived; 'New Project' butonu. Kart tıklanınca TAM SAYFA proje çalışma alanı açılır; sekmeleri: Overview (müşteri + proje bilgisi + workflow onayı + notlar), Brief (tek büyük brief kutusu + References + Pages/Structure + Work Notes + asset checklist), Budget (maliyet dökümü), Roadmap (aşamalar), Files (dosyalar). İş akışı aşamaları lead → proposal → active → review → approved. Ajans akışı: Inbox'taki lead → Proposal (fiyat teklifi) → kabulde Project; Clients bu akıştan otomatik oluşan rehberdir.",
  "CLIENTS: sade müşteri tablosu (müşteri, mesaj/teklif/proje sayıları, son iletişim, detay). Üstte tek satır özet; ayrı istatistik kartları yok. Müşteriler mesajlardan otomatik oluşur; detayda geçmiş mesajlar/teklifler/projeler.",
  "INTELLIGENCE (Visitor Intelligence): kartlar (toplam ziyaretçi, şu an online, en çok gelen konum, mobil yüzdesi), 3D dünya haritası, Live Signal Feed oturum tablosu, toplu silme ve yenileme. Veriler site ziyaretçi izleyicisinden gelir.",
  "SYSTEM (Settings): Telegram bot ayarları (token/chat), Security Protocol, passkey yönetimi, Emergency (oturumu sonlandırma)."
].join("\n");
...
// Free-tier Gemini quotas are per model, so when the primary model is rate
// limited (429) we fall through this chain instead of losing chat entirely.
...
function isQuotaError(error) {
  ...
  return text.includes("429") || /quota|rate.?limit/i.test(text);
}

let lastGeminiFailure = null;

async function generateWithFallback(buildModel, runModel) {
  let lastError = null;
  for (const modelName of geminiModelChain()) {
    try {
      const result = await runModel(buildModel(modelName));
      lastGeminiFailure = null;
      return result;
    } catch (error) {
      lastError = error;
      if (!isQuotaError(error)) break;
      console.warn(`[agent] ${modelName} rate limited, trying next model`);
    }
  }
  throw lastError;
}
...
async function geminiChatReply(text, context, history) {
  ...
  if (!apiKey) {
    console.warn("[agent] GEMINI_API_KEY is not configured; conversational chat disabled");
    lastGeminiFailure = "missing_key";
    return null;
  }
  try {
    ...
    const buildModel = (modelName) => genAI.getGenerativeModel({
      ...
      systemInstruction: [
        ...
        `PANEL REHBERİ (bölümler ve içindekiler):\n${PANEL_GUIDE}`,
    ...
    const result = await generateWithFallback(
      buildModel,
      (model) => model.generateContent({ contents }, { timeout: 12000 })
    );
    ...
    lastGeminiFailure = isQuotaError(error) ? "quota" : "error";
    return null;
  }
}
...
async function dailyChat({ text, actorId = "unknown", now = new Date() }) {
  ...
  const geminiReply = await geminiChatReply(text, context, history);
  let reply = geminiReply || deterministicChatReply(context, text);
  if (!geminiReply) {
    // Surface WHY the reply is canned so a missing/broken model is visible in
    // the panel instead of silently degrading to the briefing text.
    if (!process.env.GEMINI_API_KEY) {
      reply += "\n\n⚠️ Sohbet modeli devre dışı: Vercel'de GEMINI_API_KEY tanımlı değil. Google AI Studio'dan bir anahtar alıp Vercel > Settings > Environment Variables'a ekleyin (Production + Preview), sonra redeploy edin.";
    } else if (lastGeminiFailure === "quota") {
      reply += "\n\n⚠️ Günlük ücretsiz Gemini kotası doldu (tüm yedek modeller dahil); yukarıdaki özet sabit verilerden üretildi. Kota her gün sıfırlanır. Kalıcı çözüm: aistudio.google.com üzerinden faturalandırmayı açın (kullandıkça öde, çok düşük maliyet).";
    } else {
      reply += "\n\n⚠️ Sohbet modeline şu anda ulaşılamadı; yukarıdaki özet sabit verilerden üretildi. Ayrıntı Vercel loglarında.";
    }
  }
...
// Only messages that look like an action request are worth an extra model
// call; plain questions/chit-chat go straight to the chat model. This halves
// daily quota usage on the free tier.
function looksLikeActionRequest(text) {
  return /(ekle|ekler|kaydet|oluştur|olustur|planla|tamamla|bitir|yapıldı|yapildi|not\b|randevu|toplantı|toplanti|etkinlik|görev|gorev|to-?do|yapılacak|yapilacak|hatırlat|hatirlat|listele)/i
    .test(String(text).toLocaleLowerCase("tr-TR"));
}
...
async function geminiClassifyIntent(text, now = new Date()) {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) {
    console.warn("[agent] GEMINI_API_KEY is not configured; intent classification uses regex only");
    return null;
  }
  ...
  try {
    ...
    const result = await generateWithFallback(
      buildModel,
      (model) => model.generateContent(String(text).slice(0, 1000))
    );
