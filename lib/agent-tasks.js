...
async function collectPanelStats() {
  ...
  const [messages, unreadMessages, quotes, clients, projects] = await Promise.all([
    countOf(db.collection("messages")),
    countOf(db.collection("messages").where("read", "==", false)),
    countOf(db.collection("quotes")),
    countOf(db.collection("clients")),
    countOf(db.collection("projects"))
  ]);
  return { messages, unreadMessages, quotes, clients, projects };
}

function formatStatsLine(stats) {
  if (!stats) return "Panel istatistikleri şu anda okunamıyor.";
  const value = (n) => (n === null || n === undefined ? "?" : n);
  return `Toplam mesaj: ${value(stats.messages)} (okunmamış: ${value(stats.unreadMessages)}) · Teklif: ${value(stats.quotes)} · Müşteri: ${value(stats.clients)} · Proje: ${value(stats.projects)}`;
}
...
async function buildDailyContext(now = new Date()) {
  ...
  const [todos, agenda, recentLeads, approvals, stats] = await Promise.all([
    listTodos().catch(() => []),
    listAgenda({ now }).catch(() => []),
    agent.getRecentLeadMessages(5).catch(() => []),
    agent.listPendingApprovals().catch(() => []),
    collectPanelStats().catch(() => null)
  ]);
  ...
  try {
    ...
    const model = genAI.getGenerativeModel({
      ...
      systemInstruction: [
        ...
        `Bugün: ${context.todayKey}`,
        `Panel istatistikleri: ${formatStatsLine(context.stats)}`,
