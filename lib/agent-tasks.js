...
async function addLeadNote({ text, actorId = "unknown" }) {
  ...
  const note = extractNoteText(text) || String(text || "").trim().slice(0, MAX_TITLE);
...
async function handleTaskCommand(command, actor, now = new Date()) {
  ...
  let result;
  if (command.intent === INTENTS.LEAD_NOTE) {
    const noteResult = await addLeadNote({ text: command.text, actorId });
    result = {
      intent: command.intent,
      message: `Son lead'e (${noteResult.leadName}) not eklendi: "${noteResult.note}"`,
      note: noteResult
    };
  } else if (command.intent === INTENTS.TODO_ADD) {
