...
const { INTENTS, validationError } = require("./agent-validation");
...
async function buildDailyContext(now = new Date()) {
  const [todos, agenda] = await Promise.all([
    listTodos().catch(() => []),
...
function deterministicChatReply(context) {
  const lines = [
...
async function dailyChat({ text, actorId = "unknown", now = new Date() }) {
  ...
  const reply = (await geminiChatReply(text, context, history)) || deterministicChatReply(context);
...
async function recordTaskCommand({ command, actorId, intent, resultSummary }) {
  try {
    ...
    batch.set(db.collection("agent_audit_logs").doc(), {
      ...
      source: "agent",
      action: intent,
      targetType: intent.startsWith("todo") ? "agent_todos" : intent.startsWith("event") || intent === "agenda_list" ? "agent_events" : "agent_chats",
      targetId: null,
      risk: "low",
      result: "success",
...
async function handleTaskCommand(command, actor, now = new Date()) {
  ...
  let result;
  if (command.intent === INTENTS.TODO_ADD) {
    const { dateKey } = parseTurkishDateTime(command.text, now);
    const title = stripKeywords(command.text, [/(tamamla|bitir|done|complete)/gi]);
    const todo = await addTodo({ title, dueDateKey: dateKey, actorId });
    result = { intent: command.intent, message: `Yapılacaklara eklendi: ${todo.title}`, todo };
  } else if (command.intent === INTENTS.TODO_COMPLETE) {
    const matchText = stripKeywords(command.text, [/(tamamla|tamamlandı|tamamlandi|bitir|bitti|yapıldı|yapildi|done|complete|olarak|işaretle|isaretle)/gi]);
    const todo = await completeTodo({ matchText, actorId });
    result = { intent: command.intent, message: `Tamamlandı: ${todo.title}`, todo };
  } else if (command.intent === INTENTS.TODO_LIST) {
    const todos = await listTodos();
    result = { intent: command.intent, message: formatTodoLines(todos), todos };
  } else if (command.intent === INTENTS.EVENT_ADD) {
    const { dateKey, time } = parseTurkishDateTime(command.text, now);
    const title = stripKeywords(command.text);
    const event = await addEvent({ title, dateKey, time, actorId });
    result = { intent: command.intent, message: `Ajandaya eklendi: ${event.dateKey}${event.time ? ` ${event.time}` : ""} — ${event.title}`, event };
  } else if (command.intent === INTENTS.AGENDA_LIST) {
    const events = await listAgenda({ now });
    result = { intent: command.intent, message: formatAgendaLines(events), events };
  } else {
