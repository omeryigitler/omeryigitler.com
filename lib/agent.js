...
async function getRecentLeadMessages(limit = 5) {
...
async function getLatestLeadMessage() {
  const leads = await getRecentLeadMessages(1);
  return leads[0] || null;
}
...
async function handleCommand(input) {
  ...
  if (command.intent !== SUPPORTED_INTENT) {
    const tasks = require("./agent-tasks");
    return tasks.handleTaskCommand(command, input.actor);
  }
