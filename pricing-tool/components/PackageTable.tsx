...
const PackageTable: React.FC<Props> = ({ language }) => {
  ...
  const packageMeta = [
    ...
    { key: 'business', priceTR: '25.000 ₺', priceMT: '€2,500', color: 'border-yellow-400/50 bg-zinc-900 shadow-[0_0_30px_rgba(250,204,21,0.1)] scale-105 z-10', btnColor: 'bg-yellow-400 text-black hover:bg-yellow-300 font-bold', checks: [true, true, true, true, true, true, true] },
      ...
      <div className="text-center mb-20">
        ...
        <div className="mt-8 inline-flex bg-zinc-900 rounded-xl p-1.5 border border-zinc-800">
          <button 
            ...
            className={`px-6 py-2.5 rounded-lg text-sm font-bold transition-all ${activeCountry === Country.TR ? 'bg-yellow-400 shadow-sm text-black' : 'text-zinc-500 hover:text-white'}`}
          >
            Türkiye (₺)
          </button>
          <button 
             onClick={() => setActiveCountry(Country.MT)}
             className={`px-6 py-2.5 rounded-lg text-sm font-bold transition-all ${activeCountry === Country.MT ? 'bg-yellow-400 shadow-sm text-black' : 'text-zinc-500 hover:text-white'}`}
          >
            Malta (€)
          </button>
        </div>
        ...
        {packageMeta.map((pkg: any) => {
            ...
            return (
              <div key={pkg.key} className={`relative rounded-3xl p-10 transition-transform border ${pkg.color}`}>
                {pkgTrans.badge && (
                  <span className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-yellow-400 text-black text-xs font-bold px-4 py-1.5 rounded-full uppercase tracking-wide shadow-lg">
                    {pkgTrans.badge}
                  </span>
                )}
                ...
                <ul className="space-y-4 mb-10">
                  {pkgTrans.features.map((featureName: string, idx: number) => (
                    <li key={idx} className="flex items-start">
                      {pkg.checks[idx] ? (
                        <Check className="w-5 h-5 text-yellow-400 mr-3 flex-shrink-0" strokeWidth={3} />
                      ) : (
